...
-- I make error and all data has duplicate
-- I check the duplicate
...
-- I check again if the duplicate is already removed

SELECT company, location, industry, total_laid_off, percentage_laid_off,
       date, stage, country, funds_raised_millions, COUNT(*) AS duplicate_count
FROM layoffs_staging2
GROUP BY company, location, industry, total_laid_off, percentage_laid_off,
         date, stage, country, funds_raised_millions
HAVING COUNT(*) > 1;
...
-- Now all the duplicate is removed and i have clear data